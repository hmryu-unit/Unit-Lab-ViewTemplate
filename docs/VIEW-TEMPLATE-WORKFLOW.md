...
| **② 카테고리 · 공종** | UI ↔ `builtInCategory` ↔ 공종·자재·**모듈러 공정** (`process-database.json`) |
...
- `nodes[]`: `id`, `name`, `revitTemplateName`, `role`, `parentId`, `inheritParentVg`, `viewType`, `properties`, `processId?` (모듈러 공정 참조)
...
### `category-registry.json` (schema v2)

- `builtInCategory` — **`important: Y` 행은 반드시** 있어야 Preset export 가능 (P2)
- `categories[].processIds[]` — Revit 카테고리 ↔ 모듈러 공정 N:M
- `processes[]` — `process-database.json` 과 동기 (또는 시드 로드 시 병합)

### `process-database.json`

- 출처: `docs/공정 DB *.csv` → `python scripts/import-process-db.py`
- `processes[]`: `id`, `name`, `level` (`group`|`task`), `parentId`, `predecessorIds`, `successorIds`, `tradeIds`, `description`, …
- 웹 ② 탭: 공정 트리·선후행 조회, 카테고리·VT(`processId`) 연결
...
2. `template-library.json`, `vg-checklist.json`, `category-registry.json` 각각 다운로드 (공정은 `process-database.json` 또는 registry에 포함)
3. CSV 변경 시 `python scripts/import-process-db.py` → `Web/process-database.json` + `process-database-data.js`
4. `Web/`에 덮어쓰기 → `scripts/*` 로 `*-data.js` 재생성 → 커밋 → 배포
